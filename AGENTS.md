...
- Prefer running project workflows via the Makefile when available (e.g. `make build`, `make test`, `make format`, `make lint`). Only fall back to raw `swift` / `xcodebuild` commands when there is no Make target.
